...
export function ScriptContentCard({
  ...
  if (summaryOnly) {
    return (
      <div className={cn('px-3 py-2 text-xs', className)}>
        <p className="font-medium text-text">{script.title.replace(/ · 方案\d+$/, '')}</p>
        <p className="mt-0.5 text-text-secondary">
          {DIFFICULTY_LABEL[script.difficulty]} · {scriptSummary(script)} · 道具：
  ...
  return (
    <div className={cn('rounded-lg border border-border bg-white', compact ? 'p-3 text-xs' : 'p-4 text-sm', className)}>
      ...
      <p className={cn('mt-2 rounded-md bg-slate-50 px-2 py-1.5 text-text-secondary', compact && 'text-[11px]')}>
        {script.instruction}
      </p>
